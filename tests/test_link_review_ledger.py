...
def test_machine_api_endpoint_detection_rejected_urls(url: str) -> None:
    assert is_machine_api_endpoint(url)
...
def test_machine_api_endpoint_detection_accepted_urls(url: str) -> None:
    assert not is_machine_api_endpoint(url)


def test_link_review_ledger_candidates_and_strict_counts() -> None:
    rows = parse_strict_ledger_rows(LEDGER_PATH)
...
def test_strict_ledger_parser_validates_required_headers(tmp_path: Path) -> None:
    ledger = tmp_path / "ledger.md"
    ledger.write_text(
        "\n".join(
            [
                "| tool_id | checked_url | result_category |",
                "| --- | --- | --- |",
                "| demo | https://example.com | manual-verification-required |",
            ]
        ),
        encoding="utf-8",
    )
    ...
    with pytest.raises(AssertionError) as exc_info:
        parse_strict_ledger_rows(ledger)

    msg = str(exc_info.value)
    assert str(ledger) in msg
    assert "missing required header columns" in msg
    assert "candidate_replacement_url" in msg


def test_strict_ledger_parser_reports_actionable_row_shape_error(
    tmp_path: Path,
) -> None:
    ledger = tmp_path / "ledger.md"
    ledger.write_text(
        "\n".join(
            [
                "| tool_id | checked_url | result_category | candidate_replacement_url |",
                "| --- | --- | --- | --- |",
                "| demo | https://example.com | manual-verification-required |",
            ]
        ),
    ...
    msg = str(exc_info.value)
    assert str(ledger) in msg
    assert "expected 4 columns" in msg
    assert "got 3" in msg
